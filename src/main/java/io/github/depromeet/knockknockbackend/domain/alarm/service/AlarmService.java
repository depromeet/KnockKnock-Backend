...
public class AlarmService {
    ...
    public QueryAlarmListResponse queryList() {
        List<QueryAlarmListResponseElement> result = alarmRepository.findByReceiveUserIdAndIsActivateIsTrue(SecurityUtils.getCurrentUserId())
                .stream().map(alarm ->
                    new QueryAlarmListResponseElement(alarm.getSendUserId(), alarm.getSendUserProfile(), alarm.getTitle(), alarm.getContent(), alarm.getType(), alarm.getCreatedAt())
    ...
    public int queryCount() {
        return alarmRepository.findByReceiveUserIdAndIsActivateIsTrue(SecurityUtils.getCurrentUserId()).size();
    }
